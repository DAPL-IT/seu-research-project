<template>
  <main id="main" class="main__margin--y">
    <section class="property-grid grid">
      <div class="container">
        <div class="text-center" v-if="adsStore.isSearching">
          <div class="spinner-border"></div>
        </div>
        <div
          class="alert alert-primary"
          role="alert"
          v-else-if="!adsStore.isSearching && adsStore.searched_ads.length == 0"
        >
          No Data Found
        </div>
        <template v-else>
          <div class="row">
            <div v-for="ad in adsStore.searched_ads" :key="ad.id" class="col-md-6 col-lg-4 mb-4">
              <div class="card">
              ...
              <nav class="pagination-a">
                <ul class="pagination justify-content-end">
                  <li class="page-item" :class="[{ disabled: adsStore.currPage == 1 }]">
                    <button
                      class="page-link"
                      @click="adsStore.search({ ...route.query, page: adsStore.currPage - 1 })"
                    >
                  ...
                  >
                    <button
                      class="page-link"
                      @click="adsStore.search({ ...route.query, page: number })"
                    >
                      {{ number }}
                    </button>
                  </li>
...
onMounted(() => {
  if (adsStore.searched_ads.length == 0) {
    adsStore.search({ ...route.query, page: 1 })
  }
})
</script>
